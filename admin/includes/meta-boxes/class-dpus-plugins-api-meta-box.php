...
if ( ! class_exists( 'DPUS_Plugins_Api_Meta_Box', false ) ) {

	/**
	 * Hosting options Meta Box for a Hosted Plugin.
	 */
	final class DPUS_Plugins_Api_Meta_Box extends Meta_Box {
		...
		final protected static function render() {
			...
			$Plugin->get_admin_module()->get_template( 'plugins-api-meta-box', $Meta_Box->args );
		}
		...
		final protected static function save_post( int $post_id, \WP_Post $Post = null ) {
			$Plugins_Api_Meta = ( new DPUS_Hosted_Plugin( $Post ) )->get_plugins_api_meta();
			$data = $_POST[ DPUS_Hosted_Plugin::post_type() ]['plugins_api'];

			// Remove unnecessary posted data, set unset data.
			$data = Utility::guided_array_merge( $Plugins_Api_Meta->export_meta(), $data );
			
			$Plugins_Api_Meta->donate_link = $data['donate_link'];
			$Plugins_Api_Meta->sections = $data['sections'];
			$Plugins_Api_Meta->save_meta();
			return $post_id;
		}
