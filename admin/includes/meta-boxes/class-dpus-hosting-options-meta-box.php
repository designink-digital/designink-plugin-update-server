...
if ( ! class_exists( 'DPUS_Hosting_Options_Meta_Box', false ) ) {

	/**
	 * Hosting options Meta Box for a Hosted Plugin.
	 */
	final class DPUS_Hosting_Options_Meta_Box extends Meta_Box {
		...
		final protected static function render() {
			...
			$Plugin->get_admin_module()->get_template( 'hosting-options-meta-box', $Meta_Box->args );
		}
		...
		final protected static function save_post( int $post_id, \WP_Post $Post = null ) {
			$Hosting_Meta = ( new DPUS_Hosted_Plugin( $Post ) )->get_hosting_options_meta();
			$data = $_POST[ DPUS_Hosted_Plugin::post_type() ]['hosting_options'];

			// Inequality operator matches only key/value pairs, not order or types.
			if ( $data != $Hosting_Meta->export_meta() ) {
				// Remove unnecessary posted data, set unset data.
				$data = Utility::guided_array_merge( $Hosting_Meta->export_meta(), $data );

				$Hosting_Meta->slug = $data['slug'];
				$Hosting_Meta->github_user = $data['github_user'];
				$Hosting_Meta->github_repo = $data['github_repo'];
				$Hosting_Meta->github_token = $data['github_token'];

				$Hosting_Meta->save_meta();
				DPUS_Github_Api_Cache::clear_cache( $Hosting_Meta->slug );
			}
