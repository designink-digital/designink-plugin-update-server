...
if ( ! class_exists( 'DPUS_Plugin_Headers_Meta_Box', false ) ) {

	/**
	 * Hosting options Meta Box for a Hosted Plugin.
	 */
	final class DPUS_Plugin_Headers_Meta_Box extends Meta_Box {
		...
		final protected static function render() {
			...
			$Plugin->get_admin_module()->get_template( 'plugin-headers-meta-box', $Meta_Box->args );
		}
		...
		final protected static function save_post( int $post_id, \WP_Post $Post = null ) {
			$Headers_Meta = ( new DPUS_Hosted_Plugin( $Post ) )->get_plugin_headers_meta();
			$data = $_POST[ DPUS_Hosted_Plugin::post_type() ]['plugin_headers'];

			// Remove unnecessary posted data, set unset data.
			$data = Utility::guided_array_merge( $Headers_Meta->export_meta(), $data );

			$Headers_Meta->override_headers = isset( $_POST[ DPUS_Hosted_Plugin::post_type() ]['plugin_headers']['override_headers'] ) ? $data['override_headers'] : 'no_override';
			$Headers_Meta->author = $data['author'];
			$Headers_Meta->author_profile = $data['author_profile'];
			$Headers_Meta->homepage = $data['homepage'];
			$Headers_Meta->requires = $data['requires'];
			$Headers_Meta->tested = $data['tested'];
			$Headers_Meta->requires_php = $data['requires_php'];

			$Headers_Meta->save_meta();
			return $post_id;
		}
